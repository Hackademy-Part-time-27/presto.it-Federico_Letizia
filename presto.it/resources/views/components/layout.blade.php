...
<body>
    ...
    <div class="container-fluid">
        ...
        <a class="navbar-brand fs-1 me-5" href="/">
        ...
        <ul class="navbar-nav d-flex align-items-center justify-content-center">
            ...
            <li class="nav-item mx-1">
            <a class="nav-link fs-4 hover text-center" href="{{route('adv.index')}}">Tutti gli Annunci</a>
            </li>
            <li class="nav-item dropdown mx-1">
            <a class="nav-link dropdown-toggle fs-4 hover" href="#" id="categoriesDrop" role="button"
            data-bs-toggle="dropdown" aria-expanded="false">Categorie</a>
            <ul class="dropdown-menu" aria-labelledby="categoriesDrop">
                @foreach ($categories as $category)
                <li><a class="dropdown-item fs-4 hover" href="{{ route('categoryShow', compact('category'))}}">{{$category->name}}</a></li>
                @endforeach
            </li>
        </ul>
        </div>
        ...
        <ul class="navbar-nav ms-auto dropdown-menu-end bg-primary-subtle text-end">          
            <li class=" nav-item dropdown dropdown-menu-end text-end">
                <a class=" me-2 nav-link bg-info rounded text-end text-white dropdown-toggle fs-5 font-weight-bold dropdown-menu-end" href="#" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ auth()->user()->name }}</a>
